...
export default function Workouts() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error('[Workouts] Invalid URL', endpoint, e.message);
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error('[Workouts] Bad response', res.status, txt);
      ...
      try {
        ...
        const txt = await res.text();
        console.error('[Workouts] JSON parse error', jsonErr, 'text:', txt);
  ...
  const renderTable = (items) => {
    if (!items || items.length === 0) return <div className="text-muted">No workouts.</div>;
